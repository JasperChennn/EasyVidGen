...
class Trainer:
    ...
    def prepare_trainable_parameters(self):
        ...
        if self.args.use_ema:
            logger.info(f"Initializing EMA with decay={self.args.ema_decay}")
            self.ema_transformer = EMAModel(
            ...
            # self.ema_transformer.to(self.accelerator.device)
            if self.args.offload_ema:
                # keep EMA model in CPU and use pinned memory
                self.ema_transformer.pin_memory()
                logger.info("✅ EMA initialized with CPU offload (pinned memory)")
            else:
                # EMA 模型保持在 GPU
                self.ema_transformer.to(self.accelerator.device)
                logger.info("✅ EMA initialized on GPU")
        else:
        ...
        for epoch in range(first_epoch, self.args.num_train_epochs):
            ...
            for step, batch in enumerate(self.train_dataloader):
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if self.args.use_ema:
                        if args.offload_ema:
                            self.ema_transformer.to(device="cuda", non_blocking=True)
                        self.ema_transformer.step(self.transformer.parameters())
                        if args.offload_ema:
                    ...
                    if self.accelerator.distributed_type == DistributedType.DEEPSPEED or self.accelerator.is_main_process:
                        ...
                        if global_step % self.args.validation_epochs == 0 and self.accelerator.is_main_process:
                            ...
                            if self.args.use_ema:
                                if self.args.offload_ema: # -> gpu
                                    self.ema_transformer.to(self.accelerator.device)

                                self.ema_transformer.store(self.transformer.parameters())
                                self.ema_transformer.copy_to(self.transformer.parameters())
                            ...
                            if self.args.use_ema:
                                self.ema_transformer.restore(self.transformer.parameters())
                            self.transformer.train()
    ...
    def __prepare_saving_loading_hooks(self):
        def save_model_hook(models, weights, output_dir):
            if self.accelerator.is_main_process:
                if self.args.use_ema:
                    self.ema_transformer.save_pretrained(os.path.join(output_dir, "ema_transformer"))
                    logger.info(f"✅ Saved EMA to {os.path.join(output_dir, 'ema_transformer')}")
                for model in models:
                    if isinstance(
                        ...
                        logger.info(f"Saved state to {os.path.join(output_dir, 'transformer')}")
                    else:
        ...
        def load_model_hook(models, input_dir):
            ...
            if self.args.use_ema:
                ...
                if os.path.exists(ema_path):
                    load_model = EMAModel(ema_path, WanTransformer3DModel, foreach=self.args.foreach)
                    self.ema_transformer.load_state_dict(
            ...
            for _ in range(len(models)):
                ...
                load_model = WanTransformer3DModel.from_pretrained(input_dir, subfolder="transformer", torch_dtype=torch.bfloat16)
